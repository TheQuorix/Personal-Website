...
func (c Client) Fetch(ctx context.Context) (Track, error) {
	reqURL := c.getUrl(c.cfg.LastFmUser, c.cfg.LastFmApiKey)

	request, err := http.NewRequestWithContext(ctx, http.MethodGet, reqURL, nil)
	if err != nil {
		return Track{}, fmt.Errorf("build request: %w", err)
	}

	response, err := c.httpClient.Do(request)
	if err != nil {
		return Track{}, fmt.Errorf("request failed: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return Track{}, fmt.Errorf("read body: %w", err)
	}

	if response.StatusCode != http.StatusOK {
		return Track{}, fmt.Errorf("unexpected status %d: %s", response.StatusCode, body)
	}

	var lastfmResponse lastfmResponse
	if err := json.Unmarshal(body, &lastfmResponse); err != nil {
		return Track{}, fmt.Errorf("unmarshal: %w", err)
	}
